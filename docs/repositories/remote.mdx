...
[An example remote repository can be viewed being hosted on GitHub](https://github.com/wizardopstechTech/conjure-get-started). The directory structure is the same as a local repository, but with the additional requirement of an `index.json` file.
## Index Structure
